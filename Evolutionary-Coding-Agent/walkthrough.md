...
*   **Unit Tests**: Bổ sung tệp `pytest.ini` ở thư mục gốc để cấu hình pytest bỏ qua thư mục chứa các kịch bản nháp `scratch/` trong quá trình thu thập test case.
    Chạy lệnh pytest thông thường với API Key được thiết lập vượt qua thành công toàn bộ **35/35 bài test** mà không gặp lỗi:
    ```
    ============================= 35 passed in 22.13s =============================
    ```
...
## 4. Phase 7: Offline Dreaming & Session Distillation

Chúng tôi đã thiết kế và triển khai phân hệ **Offline Dreaming & Session Distillation** hoàn chỉnh nhằm giải quyết bài toán cồng kềnh/nhiễu của raw trace logs:

### Các thành phần chính
- **Deterministic Compression (`filters.py`)**: Loại bỏ các bước thử lại (retries), lược bỏ mã nguồn của các task đã Passed, chỉ giữ lại chi tiết thông điệp lỗi và tối đa 40 dòng đầu tiên của code lỗi của các task Failed. Đạt tỷ lệ nén vết chạy thực tế $\ge 10x$.
- **LLM Distillation (`dream_distiller.py`)**: Sử dụng DeepSeek để phân tích hành vi lỗi và tổng hợp thành cấu trúc bài học `DreamResult` bằng tiếng Việt.
- **Safety Governance (`DRM_SAFE_001`, `DRM_SAFE_002`)**:
  - Lọc bỏ các bài học có độ tự tin `< 0.6` hoặc thiếu thông tin minh chứng `evidence_task_ids`.
  - Hỗ trợ dán nhãn phạm vi bài học (`scope: task/global`) và domain của bài học (`smtp`, `regex`...).
  - Dream Loader tự động đối chiếu nếu là `task`-scoped hoặc có miền domain cụ thể để chỉ nạp bài học thích hợp vào context của task hiện tại, chống tình trạng rò rỉ cheat/hack chéo domain (ví dụ SMTP mock rules không bị rò rỉ sang Regex task).
