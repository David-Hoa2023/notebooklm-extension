...
> *Nhật ký chạy (`trace.jsonl`) quá dài — có cách **chắt lọc** thành bài học ngắn, nạp an toàn vào phiên sau mà không làm nhiễu bài khác không?* (§8 — Offline Dreaming)

Đó chính là kịch bản **Dr. Lin** trong tài liệu dự án — gồm **bằng chứng đo lường** (*run-all*, PG/SG/GG), **khám phá an toàn** (*safe explore*, cổng oracle), **học bền** (*durable skills*, ngân hàng kỹ năng), và **chưng cất offline** (*offline dreaming*, Phase 7):
...
6. Sau phiên dài, **chưng cất offline** (*dream*) biến nhật ký thành bài học gọn — rồi **nạp có chọn lọc** vào phiên kế tiếp.

Kịch bản mô tả cả trường hợp xấu (đề kiểm tra sai → bị loại) lẫn trường hợp tốt (quá cổng, giải thành công, skill được lưu). Tháng 6/2026, nhóm dự án đã **chạy thật** pipeline đo lường (*run-all*, 6 seed), explore (*seed 42*), và đánh giá A/B Offline Dreaming (*seeds 42, 43*) — kết quả dưới đây.
...
### Ba con số chính (trung bình 6 seed - DeepSeek)

| Chỉ số | Ý nghĩa đơn giản | Kết quả |
|--------|------------------|---------|
| **Plasticity Gain (PG)** | Có giúp **khi đang học** không? | **0,000** — trần hiệu năng |
| **Stability Gain (SG)** | Có **ổn định** sau khi đóng băng bộ nhớ? | **0,000** — ổn định tuyệt đối |
| **Generalization Gain (GG)** | Có **áp dụng** sang bài chưa thấy? | **0,000** — trần hiệu năng |

**Đọc nhanh:** Cả 3 chỉ số đạt 0.000 do tất cả các bài tập trong curriculum huấn luyện và held-out đều đã được giải quyết hoàn hảo đạt điểm số tối đa `9.0` ngay từ vòng Baseline. Khi baseline đã chạm trần (ceiling saturation), bộ nhớ tiến hóa không có dư địa để cải thiện thêm điểm số.

### Giả thuyết thống kê — thẳng thắn

Dashboard đo lường hai giả thuyết đã đăng ký trước (H1, H2):

| Giả thuyết | Kết luận buổi chạy mới nhất |
|------------|----------------------------|
| **H1** — điểm vòng đóng băng cao hơn vòng không nhớ | **Không ý nghĩa** (p = 0.500, t = 0.00); trung bình điểm bằng nhau tuyệt đối ở mức trần 9.0. |
| **H2** — điểm ổn định sau đóng băng | **Không ý nghĩa** (p = 0.500) do điểm số đi ngang không đổi. |

Đặc biệt, bài toán **Negative Transfer** (SMTP `NEG_001`) vốn trước đây bị điểm 0.0 do nhiễu bộ nhớ cấu hình xác thực và mock context manager, nay đã được **khắc phục triệt để**, giải quyết thành công đạt điểm số tối đa **9.0** trên toàn bộ 6 seed cho cả baseline và robustness (với memory).

> **Kết luận §1:** Evolutionary Coding Agent sử dụng mô hình DeepSeek đã chứng minh độ tin cậy vận hành rất cao. Ở curriculum hiện tại, điểm số baseline đã đạt mức trần tuyệt đối (9.0), do đó hiệu quả gia tăng của bộ nhớ chưa thể đo lường được (null effect). Để đo được PG > 0, Tiến sĩ Lin cần thiết kế các bài tập khó hơn hoặc siết chặt bộ tiêu chí kiểm thử (judge) để hạ điểm baseline.
...
## Offline Dreaming — chắt lọc bài học từ nhật ký (Phase 7)

Sau nhiều giờ chạy, file `logs/trace.jsonl` có thể dài hàng nghìn dòng: mã nguồn, lần thử lại, lời giải thích của “giám khảo” AI. **Không ai muốn** ném toàn bộ vào prompt lần sau — vừa tốn token, vừa dễ **rò rỉ** kinh nghiệm sai chỗ (ví dụ quy tắc SMTP mock lọt sang bài regex).

**Offline Dreaming** giải quyết đúng bài toán đó:

| Bước | Việc hệ thống làm | Giống đời thường |
|------|-------------------|------------------|
| **Nén** | Bỏ retry, rút gọn log task đã pass | Tóm tắt biên bản cuộc họp |
| **Chưng cất** | DeepSeek đọc trace, viết bài học ngắn (tiếng Việt) | Giáo viên soạn “3 điều rút ra” |
| **Lọc an toàn** | Bỏ insight tự tin thấp; bắt buộc có bằng chứng task | Không tin lời đồn không nguồn |
| **Nạp có chọn lọc** | Chỉ insight đúng *phạm vi* và *miền* (regex, smtp…) | Chỉ áp dụng mẹo đúng môn học |
| **Tự phục hồi DB** | Xóa database → tự nạp lại từ file JSON | Sao lưu sách giáo khoa trên ổ cứng |

### Thí nghiệm A/B (2 seed: 42 và 43)

Tiến sĩ Lin chạy **hai vòng** giống nhau: lần một **tắt** dreaming, lần hai **bật** sau khi chưng cất bài học từ trace.

| Chỉ số | Tắt dreaming | Bật dreaming | Ý nghĩa |
|--------|--------------|--------------|---------|
| Điểm bài huấn luyện | 9,0 | 9,0 | Vẫn chạm **trần** — không kỳ vọng tăng điểm |
| Bài held-out | 9,0 | 9,0 | Tổng quát hóa ổn định |
| NEG_001 (SMTP) | 9,0 | 9,0 | **Không regression** — lọc miền hoạt động |
| PG / SG | 0,000 | 0,000 | Hiệu ứng null tại trần |
| Bài học được nạp | 0 | **18** lần trên **6** lượt chạy | Dây nối trace → prompt **có bằng chứng** |

> **Kết luận §8:** Ở curriculum hiện tại, Offline Dreaming **không “nâng điểm”** thêm — vì baseline đã 9,0. Giá trị thật là **hạ tầng tin cậy**: chắt lọc log ồn ào, nạp an toàn, ghi nhận trong trace, hiển thị trên dashboard Phase 7. Khi bài khó hơn làm baseline tụt điểm, lớp bài học này đã sẵn sàng.

**Kiểm tra nhanh:** `python scratch/verify_dream_session.py` — năm mục `[PASS]`; dashboard → thẻ *Phase 7: Offline Dreaming*.
...
Dashboard còn nhiều phần khác (điểm từng bài, giả thuyết thống kê H1/H2, chi phí token, **Phase 7 Offline Dreaming**…) — **Phase 5** là nơi Tiến sĩ Lin nhìn đầu tiên khi đánh giá buổi khám phá an toàn.

---

## Tại sao dự án này khác — và đáng chú ý?

Giữa vô số demo AI viết code, **Evolutionary Coding Agent** nổi bật ở **sáu** điểm có thể giải thích mà không cần đọc source code:
...
- Agent làm tốt hơn khi **được nhớ** kinh nghiệm cũ không? → PG **0,000** (bằng điểm baseline do hiệu ứng biên trần)
- Khi **đóng băng** bộ nhớ, agent có **ổn định** không? → SG **0,000** (điểm số ổn định tuyệt đối ở mức trần)
- Agent có **áp dụng** được sang bài chưa thấy không? → GG **0,000**
...
### 6. Nhật ký dài được “mơ” thành bài học ngắn

Thay vì nhét cả trace vào prompt, hệ thống **chưng cất offline**: nén log, distill bằng LLM, lọc theo độ tin cậy và miền, rồi nạp có chọn lọc. Thí nghiệm A/B (2 seed) chứng minh **18 lần nạp bài học trên 6 lượt chạy** mà **không làm tụt điểm** và **không gây nhiễu chéo** (SMTP vẫn 9,0). Đây là bước tiến từ “có bộ nhớ” sang “bộ nhớ **có kiểm soát**”.
...
> Chúng ta không chỉ hỏi AI “viết giúp tôi một hàm”. Chúng ta xây một **giáo viên ảo** biết tự ra bài, tự chấm, **lưu skill đã kiểm chứng**, **chắt lọc bài học từ nhật ký**, tự làm lại bài cũ — và quan trọng nhất: **có cơ chế từ chối bài hỏng trước khi thi**.
...
- **35/35** bài kiểm thử tự động pass (gồm 9 test Offline Dreaming).
- **100%** năng lực cốt lõi có skill đã xác minh; **35** skill trong DB, **24** đang active (*retrievable*).
- Vòng **khám phá chủ động** (Phase 5), **bằng chứng đo lường** (Phase 6), và **Offline Dreaming** (Phase 7) hoàn thiện ở mức demo nghiên cứu.
- **`run-all` 6 seed** (DeepSeek): PG 0,000, SG 0,000, GG 0,000; H1/H2 **chưa** đạt ý nghĩa thống kê (p = 0,500) do hiệu ứng biên trần 9,0.
- **A/B Offline Dreaming** (seeds 42, 43): 18 bài học nạp trên 6 lượt chạy, **không regression**, NEG_001 vẫn **9,0**.
- Bộ nhớ skill đã **sửa chữa và sao lưu** sau lỗi merge hiếm — minh chứng vận hành lâu dài.
- Log thí nghiệm tại `logs/trace.jsonl`; dashboard Phase 7 tại `logs/dashboard.html`.

Thách thức tiếp theo: (1) phá vỡ mức trần 9,0 — curriculum khó hơn hoặc judge nghiêm hơn; (2) **skill extraction** — một số bản trích xuất vẫn lỗi định dạng; (3) đo **PG > 0** khi baseline tụt điểm — lớp Offline Dreaming đã sẵn sàng.
...
- **Dashboard** giữ lời hứa bằng số liệu — kể cả thẻ **Phase 7: Offline Dreaming**.
- **Chưng cất offline** biến nhật ký dài thành bài học ngắn, nạp an toàn.

Đó không phải tương lai xa. Đó là những gì đã chạy được — và đang được ghi lại từng dòng trong nhật ký thí nghiệm.

---

*Tài liệu liên quan: [use-case.md](use-case.md) (Dr. Lin §1 run-all + §2 durable skills + §3 oracle gates + §8 offline dreaming), [walkthrough.md](walkthrough.md) (Phase 7 §4.1 A/B), [memory.md](memory.md). Báo cáo: `logs/dashboard.html`; kiểm tra §1: `scratch/verify_memory_evidence.py`; §2: `scratch/verify_durable_skills.py`; §8: `scratch/verify_dream_session.py`.*
